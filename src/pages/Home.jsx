...
const Home = () => {
  ...
  // Load news when component mounts and set up auto-refresh every 5 minutes
  useEffect(() => {
    loadNews();

    const intervalId = setInterval(() => {
      console.log('🔄 Triggering scheduled auto-refresh...');
      loadNews();
        ...
        {!loading && articles.length > 0 && (
          <div className="mb-12">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
              ...
              <span className="inline-flex items-center px-3 py-1 rounded-full text-2xs font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30 w-fit">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 mr-2 animate-pulse"></span>
                Auto-updating • Last updated: {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </span>
            </div>
